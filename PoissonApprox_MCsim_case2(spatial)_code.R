...
library(mgcv);library(spmoran);library(Matrix)
  ...
  for(n in c(50,200)){       # Sample size
    ...
    for(i in 1:nsim){
      ...
      while(dum==1){
        ...
        Xs    <- predict(mod3,type="lpmatrix")
        lpred3<- Xs%*%coef(mod3)
        pred3 <- exp(lpred3)
        wXs   <- c(sqrt(pred3))*Xs
        wy    <- c(sqrt(pred3))*(lpred3 + (y - pred3)/pred3)
        
        S_mat <-list(NULL)
        S_mat[[1]]<-matrix(0,nb,nb)
        for(iii in 1:length(mod3$sp)){
          S_mat[[iii+1]]<-mod3$sp[iii]*mod3$smooth[[iii]]$S[[1]]
        }
        V3    <- bdiag(S_mat)
        beta3 <- solve(t(wXs)%*%wXs+V3) %*% t(wXs)%*%wy
        bse3  <- sqrt(sig3*diag(solve(t(wXs)%*%wXs+V3)))
        pred3 <- as.vector(exp( Xs %*% beta3 ))
        ranef3<- Xs[,-(1:3)] %*% beta3[-(1:3)]
      ...
      B1[i,]<-c(coefficients(mod1)[1],coefficients(mod2)[1],beta3[1],coefficients(mod4)[1])
      B2[i,]<-c(coefficients(mod1)[2],coefficients(mod2)[2],beta3[2],coefficients(mod4)[2])
      B3[i,]<-c(coefficients(mod1)[3],coefficients(mod2)[3],beta3[3],coefficients(mod4)[3])
      
      ######################## Summarize coefficient standard errors
      Bse1[i,]<-c(summary(mod1)$se[1],summary(mod2)$se[1],bse3[1],summary(mod4)$se[1])
      Bse2[i,]<-c(summary(mod1)$se[2],summary(mod2)$se[2],bse3[2],summary(mod4)$se[2])
      Bse3[i,]<-c(summary(mod1)$se[3],summary(mod2)$se[3],bse3[3],summary(mod4)$se[3])
      
      ######################## Summarize spatial random effects estimates
      ranef <- cbind(predict(mod1,type="terms")[,3],predict(mod2,type="terms")[,3],ranef3,predict(mod4,type="terms")[,3])
